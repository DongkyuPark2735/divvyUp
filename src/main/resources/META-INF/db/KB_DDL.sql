...
SELECT COUNT(*) FROM NOTIFICATION_HISTORY WHERE NOTRECEIVER='ccc' AND NOTCHECK=0; 

-- 5) 사용자가 읽지 않은 알람 리스트 가져오기
SELECT * FROM NOTIFICATION_HISTORY WHERE NOTRECEIVER='ccc' AND NOTCHECK=0;

-- 6) 알람리스트를 불러와서 사용자가 확인 했을 때 NOTCHECK를 1로 업데이트 시키기
UPDATE NOTIFICATION_HISTORY SET NOTCHECK=1 WHERE NOTRECEIVER='ccc' AND NOTCHECK=0;
