...
SELECT * FROM (SELECT ROWNUM RN, A.* 
            FROM (SELECT * FROM MEMBER WHERE NOT MID='owen6113' AND MID LIKE '%'||'owen6113'||'%'ORDER BY MID) A)
...
DROP SEQUENCE NOTIH_SEQ;
DROP TABLE NOTIFICATION_HISTORY;
CREATE SEQUENCE NOTIH_SEQ MAXVALUE 9999999999 NOCYCLE NOCACHE;
CREATE TABLE NOTIFICATION_HISTORY(
	NOTIHID NUMBER(10) PRIMARY KEY, -- 시퀀스 : NOTIH_SEQ	 MAXVALUE 9999999999 NOCYCLE NOCACHE;
...
ROLLBACK;
-- 2) 'aaa'가 'bbb'를 팔로우 함으로써 'bbb' 알림창에 팔로우 알람을 발생시킴 => NOTIFICATION 테이블에 INSERT문 : INSERT INTO NOTIFICATION () VALUES (); 
INSERT INTO NOTIFICATION (NOTID, NOTSENDER, NOTRECEIVER, NOTCONTENT, NOTKIND) VALUES (NOTI_SEQ.NEXTVAL, 'aaa', 'bbb', '님이 회원님(현재 로그인한 회원)을 팔로우하기 시작했습니다', 0); 
...
INSERT INTO NOTIFICATION_HISTORY 
    (SELECT NOTIH_SEQ.NEXTVAL, NOTID, NOTSENDER, NOTRECEIVER, NOTCONTENT, NOTDATE, NOTKIND, NOTCHECK 
        FROM NOTIFICATION
            WHERE (NOT NOTID=(SELECT NOTID FROM NOTIFICATION_HISTORY)) OR (NOT EXISTS(SELECT DISTINCT NOTID FROM NOTIFICATION_HISTORY)));
ROLLBACK;
-- 4) 'bbb'가 알림창 버튼을 누른다 A(지금까지 쌓인 알림 리스트 보기) => SELECT * FROM NOTIFICATION_HISTORY WHERE notreceiver = 'bbb' AND notcheck=0;
SELECT * FROM NOTIFICATION_HISTORY WHERE NOTRECEIVER = 'bbb' AND NOTCHECK=0;
-- 5) 'bbb'가 알림창 버튼을 누른다 B(해당 알림 리스트의 읽음여부를 바꾸기) => UPDATE NOTIFICATION_HISTORY SET notcheck=1 WHERE notreceiver ='bbb' AND notcheck=0;
