...
INSERT INTO EVENTDETAIL (EDID, EID, MID, EDSHARE)
    VALUES (EVENTDETAIL_SEQ.NEXTVAL, 2, 'aaa', ((SELECT EAMOUNT FROM EVENT WHERE EID=2)/(SELECT ECOUNT FROM EVENT WHERE EID=2)));
    
    --<c:if test="member.mid != pay.mid">  
INSERT INTO EVENTDETAIL  (EDID, EID, MID, EDSHARE)
    VALUES (EVENTDETAIL_SEQ.NEXTVAL, 2, 'bbb', -1*((SELECT EAMOUNT FROM EVENT WHERE EID=2)/(SELECT ECOUNT FROM EVENT WHERE EID=2)));
...
--3.바로 GROUP DETAIL TABLE 에 멤버 정보를 업데이트 해준다
    ...
    UPDATE GROUPDETAIL 
      ...
      UPDATE GROUPDETAIL 
        SET GDBALANCE = GDBALANCE + (SELECT EDSHARE FROM EVENTDETAIL ED, GROUPDETAIL GD WHERE WHERE GD.MID = ED.MID AND EID=(SELECT MAX(EID) FROM EVENT))
        WHERE MID='aaa';
